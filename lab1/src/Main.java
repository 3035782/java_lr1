...
public class Main {
    public static void main(String[] args) {
        ...
        String s_number = String.valueOf(number); // переводим число в строку, чтобы поулчить последний символ
        System.out.println(s_number.charAt(s_number.length() - 1)); // chatAt(n) - метод, которйы возвращаем символ в строке на n месте

        // =================================================================
        System.out.println("Второе задание");
        scanner.nextLine(); // это для предотвращения проблемы nextLine после nextInt
        number = scanner.nextInt(); // чтобы не никто не ввел строку, получим сначала число
        s_number = String.valueOf(number);
        int result = 0;
        for (int i = 0; i < s_number.length(); i++) { // проходим от 0 до длинны строки
            result = result + Integer.parseInt(String.valueOf(s_number.charAt(i))); // получаем символ из строки под i, превращаем его в число и ссумируем в result
        }
